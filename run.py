...
def duplicate_check(to_check, toadd, tag_priority, article_structure, duplicates, answer, content, youtube, id, question, tag, answer_id, youtube_id):
    if to_check == "id":
...
def add_structure(article_structure, duplicates, question, tag, answer, youtube):
    ...
    if duplicate_id or duplicate_answer or duplicate_youtube:
        ...
        if duplicate_id:
            duplicate_check("id", toadd, tag_priority, article_structure, duplicates, answer, content, youtube, id, question, tag, answer_id, youtube_id)
        if id != answer_id and duplicate_answer:
            duplicate_check("content", toadd, tag_priority, article_structure, duplicates, answer, content, youtube, id, question, tag, answer_id, youtube_id)
        if id != youtube_id and duplicate_youtube:
            duplicate_check("youtube", toadd, tag_priority, article_structure, duplicates, answer, content, youtube, id, question, tag, answer_id, youtube_id)
    else:
        if content:
            duplicates["content"][content] = id
        if youtube:
...
def make_article(header):
    ...
    add_structure(article_structure, duplicates, info["question"], "h1", info["answer"], info["youtube"])
    for h2 in info["related"]:
        info_h2 = get_info(h2)
        if not info_h2:
            continue
        add_structure(article_structure, duplicates, info_h2["question"], "h2", info_h2["answer"], info_h2["youtube"])
        for h3 in info_h2["related"]:
            info_h3 = get_info(h3)
            if not info_h3:
                continue
            add_structure(article_structure, duplicates, info_h3["question"], "h3", info_h3["answer"], info_h3["youtube"])
...
#make_article("how do arenas switch from hockey to basketball")
